...
function normalizeText(text) {
  ...
  for (let pass = 0; pass < 3; pass++) {
    ...
    // Sumar "Xmil + centena": "3000 200" → "3200", "80000 500" → "80500"
    // Solo centenas (≥100) para no fusionar cuota counts o cantidades sueltas ("60000 6" no se toca)
  ...
  // Paso 3b: artefacto SR — "120 0 12" → "120000 12"
  // Ocurre cuando el SR transcribe "ciento veinte mil doce cuotas" como "120 0 12 cuotas"
  // Solo aplica si el tercer token D es 2-60 (posible cuota count)
  t = t.replace(/\b(\d{2,})\s+(0)\s+(\d{1,2})\b/g, (match, n, _z, d) => {
    const di = parseInt(d)
    return (di >= 2 && di <= 60) ? `${parseInt(n) * 1000} ${d}` : match
  })
...
const MEDIO_PAGO_PATTERNS = {
  credito:      /tarjeta|visa|master|amex|crédito|credito|naranja|galicia|santander|bbva|macro|icbc|hsbc|cuotas?/,
...
// Solo reconoce cuotas en rango razonable (2-60) para evitar confundir montos con cuotas
...
/**
 * Artefacto SR: "61500 6 cuotas" se transcribe como "615006 cuotas"
 * El SR concatena el monto y las cuotas en un solo número.
 * Si N > 999 y sus últimos 1-2 dígitos forman un número de cuotas válido (2-60)
 * y el monto resultante es múltiplo de 10, desagrega.
 */
function trySplitFused(text) {
  const m = text.match(/\b(\d{4,})\s+cuotas?\b/)
  if (!m) return null
  const N = parseInt(m[1])

  // Probar último dígito como cuotas (N = monto * 10 + cuotas)
  const last1 = N % 10
  if (last1 >= 2 && last1 <= 9) {
    const amount = Math.floor(N / 10)
    if (amount >= 100 && amount % 10 === 0)
      return { text: text.replace(m[1], String(amount)), cuotas: last1 }
  }

  // Probar últimos 2 dígitos como cuotas (N = monto * 100 + cuotas)
  const last2 = N % 100
  if (last2 >= 10 && last2 <= 60) {
    const amount = Math.floor(N / 100)
    if (amount >= 100 && amount % 10 === 0)
      return { text: text.replace(m[1], String(amount)), cuotas: last2 }
  }
...
export function parseVoice(text) {
  ...
  let cuotas = detectCuotas(processed)
  // Si no se detectaron cuotas pero hay un número grande antes de "cuotas",
  // intentar desagregar el artefacto de fusión del SR ("615006 cuotas" → 61500, 6)
  if (cuotas === 1) {
    const split = trySplitFused(processed)
    if (split) { processed = split.text; cuotas = split.cuotas }
  }
  ...
  const base      = pool.length > 0 ? pool : nums
  // Safety net: si el último elemento es "0" (artefacto SR de miles), usarlo penúltimo
  const efectivos = base.length > 1 && parseFloat(base[base.length - 1]) === 0
    ? base.slice(0, -1) : base
