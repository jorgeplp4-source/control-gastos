...
function normalizeText(text) {
  let t = text.trim().toLowerCase().replace(/[,]+/g,' ').replace(/\.+/g,' ').replace(/\s+/g,' ')
  ...
  // Unir números separados por espacio que forman miles
  // Unir si: segundo bloque empieza en 0 (ej: "30 000", "1 000")
  //       o: primer bloque tiene 2+ dígitos (ej: "10 500", "25 000")
  t = t.replace(/(\d{2,}) (\d{3})\b/g, '$1$2')   // "10 500" → "10500"
  t = t.replace(/(\d+) (0\d{2})\b/g,   '$1$2')   // "1 000"  → "1000", "5 050" → "5050"
  // Repetir para millones
  t = t.replace(/(\d{2,}) (\d{3})\b/g, '$1$2')
  return t
}
...
/**
 * Parsea transcripción en orden: CATEGORÍA/ÍTEM · CANTIDAD · MONTO · CUOTAS
...
export function parseVoice(text) {
  ...
  // Detectar cuotas y medio de pago del texto original
  const cuotas     = detectCuotas(text)
  const medio_pago = detectMedioPago(text)

  if (nums.length === 0) return { itemQuery: raw, cantidad:'', monto:'', cuotas, medio_pago }
  if (nums.length === 1) return { itemQuery, cantidad:'1', monto: nums[0], cuotas, medio_pago }

  // Si hay cuotas detectadas, el número de cuotas ya fue extraído → monto es el total
  // Evitar que el número de cuotas sea interpretado como cantidad/monto
  const numsFinanciales = nums.filter(n => {
    const v = parseFloat(n)
    return v !== cuotas || nums.length <= 2
  })

  const monto = numsFinanciales[numsFinanciales.length - 1] || nums[nums.length - 1]
  const cantidad = numsFinanciales.length > 1 ? numsFinanciales[0] : '1'

  return { itemQuery, cantidad, monto, cuotas, medio_pago }
}
